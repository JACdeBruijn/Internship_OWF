################################################################################-
## 3_Analysis, plotting & GAMs - Interaction CPOD-WBAT ----
## Joining and plotting the interaction - 15/01/25
## Running GAM models on the data 
################################################################################-
...
gam.check(CPOP_final)

plot(CPOP_final, pages = 1)

################################################################################-
# The final model for WBAT (log10(SA)) ----
################################################################################-

WBAT_final <- bam(log10_SA ~ 
                     type +
                     pph +
                     s(sunPos_altitude, k = 12) +
                     s(week_num, k = 12) +
...
gam.check(WBAT_final)

plot(WBAT_final, pages = 1)


###############################################################################--
# Notes on the entire modelling process are below here ----
################################################################################-
# Some modeling
WC_1 <- gam(log10_SA ~ 
              type +
...
# Nadat ik de mail naar jeroen heb gevonden even wat dingen testen
WC_62 <- bam(log10_SA ~ 
               type +
               pph +
               pairingName +
