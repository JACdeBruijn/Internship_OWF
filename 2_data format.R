###
# Started working through this script - 14/10/24
# Purpose of script: 
###
...
# Source all R scripts in the "Function" folder
sourceDir(file.path(".", "Function"))
...
if(processFlag){
  load(file.path(dataPath,'CPOD_all.RData'))    # loading data
  ...
  for(stationSet in unique(WBAT.tab$dataSet_station)){                              # Loop that will do everything for each unique dataSet_station that does also occur in overview.tab
    ...
    for(idxDataSet in tab.filt$surveName){                                          # Reading all the data on station and data set for pelegc fish data
      ...
      for(idxTreshold in unique(WBAT.all$treshold)){                                # Doing things that are not very understandable 
      ...
      WBAT.depth <- subset(WBAT.all,depth != 0) %>% group_by(IDinter,frequency,treshold,datetime) %>% summarize(depthSA=sum(SA*depth)/sum(SA))  # working with depth, 
    ...
    WBAT.join <- subset(WBAT.join,IDinter %in% subset(samp.summary,n >= 6)$IDinter) %>%   # Creating a subset, with min 6 entries and max 11 entries
    ...
    mySunlightTimes <- getSunlightTimes(date = as.Date(WBAT.summary$datetime),          # Coding in the sunlight times
    ...
    if(!(unique(WBAT.summary$dataSet) %in% c('2022-HKZ','2023-HKN'))){          # Renaming and splitting CPOD data
  ...
  save(WBAT.all.summary,CPOD.all.day,CPOD.all.hour,CPOD.all.min,#WBAT.sample.all                        # This is for saving all the new data frames, this is therefore also an overview for the names of the data frames
       file = file.path(resultPath,'CPOD_WBAT_workspace.RData'))                # Conclussion I can try to run everything myzelf and get the data frames that way, or I need to ask Ben for these WBAT.sum, CPOD.day, CPOD.hour, CPOD.min
}else{
